...
protected:
    ...
    void ConfigureBgpRouter() {
        ...
        a_vpn_ =
            static_cast<BgpTable *>(a_->database()->FindTable("bgp.inetmvpn.0"));
        b_vpn_ =
            static_cast<BgpTable *>(b_->database()->FindTable("bgp.inetmvpn.0"));
        a_blue_inet_ =
    ...
    void VerifyVpnTable(RoutingInstance *from_instance, string prefix,
        ...
        InetMVpnPrefix vpn(*rd, from_instance->server()->bgp_identifier(),
                rt_prefix.ip4_addr(), rt_prefix.ip4_addr());
        InetMVpnTable::RequestKey key(vpn, NULL);
...
TEST_P(L3VPNPeerTest, AsPathLoop) {
    ...
    //       Verify that path in bgp.inetmvpn.0 is marked as infeasible         //
    ///////////////////////////////////////////////////////////////////////////
...
INSTANTIATE_TEST_CASE_P(InetMVpnPeerTestWithParams, L3VPNPeerTest,

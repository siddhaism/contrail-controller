...
TEST_F(BgpAttrTest, EdgeDiscovery2) {
    ...
    for (int idx = 1; idx < 3; ++idx) {
        ...
        edge->SetAddress(Ip4Address::from_string(addr_str, ec));
    ...
    for (EdgeDiscoverySpec::EdgeList::const_iterator it =
        ...
        error_code ec;
        EXPECT_EQ(addr_str, edge->GetAddress().to_string(ec));
...
TEST_F(BgpAttrTest, EdgeForwarding2) {
    ...
    for (int idx = 1; idx < 3; ++idx) {
        ...
        edge->SetInboundAddress(Ip4Address::from_string("10.1.1.100", ec));
        edge->inbound_label = 100000;
        edge->SetOutboundAddress(Ip4Address::from_string(addr_str, ec));
    ...
    for (EdgeForwardingSpec::EdgeList::const_iterator it =
        ...
        error_code ec;
        std::string addr_str = "10.1.1." + integerToString(idx);
        EXPECT_EQ("10.1.1.100", edge->GetInboundAddress().to_string(ec));
        EXPECT_EQ(100000, edge->inbound_label);
        EXPECT_EQ(addr_str, edge->GetOutboundAddress().to_string(ec));
